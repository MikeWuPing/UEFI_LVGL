...
[![LVGL](https://img.shields.io/badge/LVGL-9.2.3-green.svg)](https://github.com/lvgl/lvgl)
...
- **LVGL v9.2.3** 以 EDK2 `BASE` 库形式编译，源码是原版上游镜像（git 子模块方式引入），无 fork、无补丁文件。
...
└── Library/
    ...
    │   └── lvgl/                   # git 子模块 → 上游 lvgl v9.2.3（原版镜像）
    └── LvglUefiPort/
...
- **LVGL v9.2.3** built as an EDK2 `BASE` library from the pristine upstream sources (vendored as a git submodule — no forks, no patched files).
...
└── Library/
    ...
    │   └── lvgl/                   # git submodule → upstream lvgl v9.2.3 (pristine mirror)
    └── LvglUefiPort/
...
The LVGL mirror is a git submodule pointing at upstream v9.2.3:
