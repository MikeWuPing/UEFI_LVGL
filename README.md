...
- **定制 `lv_conf.h`**：32bpp、`LV_STDLIB_CUSTOM` malloc + 内置 string/sprintf（不碰 libc）、`LV_OS_NONE`、33ms 刷新周期（约 30fps）、深色主题、Montserrat 12/14/16、`unscii_16` 等宽字体及可选的 18px 简体中文 UI 子集，关闭 demo 与日志。
...
└── Library/
    ...
    │   └── Fonts/                  # SimSun 字体及可复现的 18px 简体中文 UI 子集
    └── LvglUefiPort/
...
| `LV_FONT_*` | Montserrat 12/14/16 + `UNSCII_16` + `zh_cn_ui_18` | `unscii_16` 用于等宽编辑区；511 字的简体中文子集由消费者显式选择 |
| `LV_USE_LOG` / 各类 demo | `0` | 固件无日志出口；demo 编译时剔除 |

该文件刻意使用 UTF-8 并保留中文注释，`LvglLib.inf` 的编译选项带 `/utf-8`（及 `/wd4819`）供 MSVC 正确解码——请勿转码或加 BOM。

18px 简体中文字体可用 `&lv_font_zh_cn_ui_18` 选择，默认字体仍是 Montserrat 14。使用 [Source Han Sans SC 2.004R](https://github.com/adobe-fonts/source-han-sans/releases/download/2.004R/SourceHanSansSC.zip) 中 SHA-256 与元数据记录值一致的 Regular OTF，以及固定的 `lv_font_conv 1.5.3`，可执行：

```bash
python3 Tools/Fonts/generate_zh_cn_ui_18.py \
  --manifest Library/LvglLib/Fonts/zh_cn_ui_18_glyphs.txt \
  --font /path/to/SourceHanSansSC-Regular.otf \
  --output Library/LvglLib/Fonts/lv_font_zh_cn_ui_18.c \
  --metadata Library/LvglLib/Fonts/lv_font_zh_cn_ui_18.txt \
  --check --font-conv npx --yes lv_font_conv@1.5.3
python3 Tools/Fonts/verify_zh_cn_ui_18.py
```
...
`LvglPkg` 代码以 **MIT License** 发布——见 [LICENSE](LICENSE)。随包携带的 LVGL 镜像保留上游项目自身的 MIT 许可（子模块内原样保留）。`lv_font_zh_cn_ui_18` 派生自 Source Han Sans SC，遵循 [SIL Open Font License 1.1](Library/LvglLib/Fonts/OFL-SourceHanSans.txt)。
...
- **Tuned `lv_conf.h`**: 32 bpp, `LV_STDLIB_CUSTOM` malloc + built-in string/sprintf (no libc), `LV_OS_NONE`, 33 ms refresh (~30 fps), dark theme, Montserrat 12/14/16, `unscii_16` monospace, and an optional 18 px Simplified Chinese UI subset; demos/logs disabled.
...
└── Library/
    ...
    │   └── Fonts/                  # SimSun fonts and a reproducible 18 px Simplified Chinese UI subset
    └── LvglUefiPort/
...
| `LV_FONT_*` | Montserrat 12/14/16 + `UNSCII_16` + `zh_cn_ui_18` | `unscii_16` is monospace; consumers explicitly select the 511-glyph Chinese subset |
...
Select the 18 px Simplified Chinese font with `&lv_font_zh_cn_ui_18`; Montserrat 14 remains the default. With the Regular OTF from [Source Han Sans SC 2.004R](https://github.com/adobe-fonts/source-han-sans/releases/download/2.004R/SourceHanSansSC.zip) whose SHA-256 matches the metadata, and pinned `lv_font_conv 1.5.3`, reproduce and verify the asset with:
...
`LvglPkg` code is released under the **MIT License** — see [LICENSE](LICENSE). The bundled LVGL mirror retains its own MIT license from the upstream project. `lv_font_zh_cn_ui_18` is derived from Source Han Sans SC and is distributed under the [SIL Open Font License 1.1](Library/LvglLib/Fonts/OFL-SourceHanSans.txt).
