...
└── Library/
    ...
    │   └── Fonts/                  # simsun 中文字库（本包从 simsun.ttc 重生成，覆盖界面所需简体字符；镜像内原版不编译）
    └── LvglUefiPort/
...
| `LV_USE_PRIVATE_API` | `1` | 向消费方开放 `lvgl_private.h` 控件私有头文件（如 `lv_dropdown_t`），供应用读取/重置控件内部状态（LVGL v9 无公开的清除选中 API）；仅影响头文件可见性，不改库行为 |
...
└── Library/
    ...
    │   └── Fonts/                  # SimSun CJK fonts (regenerated from simsun.ttc, covering the simplified glyphs a UI needs; the mirror's originals are not compiled)
    └── LvglUefiPort/
...
| `LV_USE_PRIVATE_API` | `1` | exposes the private widget structs (`lvgl_private.h`, e.g. `lv_dropdown_t`) so consumers can inspect/reset widget state (LVGL v9 has no public clear-selection API); header visibility only, library behavior unchanged |
